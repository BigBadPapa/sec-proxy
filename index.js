...
const METRICS_CATALOG = {
  ...
  netchangeincash: { tags: ['CashCashEquivalentsRestrictedCashAndRestrictedCashEquivalentsPeriodIncreaseDecreaseIncludingExchangeRateEffect', 'CashCashEquivalentsRestrictedCashAndRestrictedCashEquivalentsPeriodIncreaseDecreaseExcludingExchangeRateEffect'], category: 'CashFlow', ttm: 'sum', ru: 'Чистое изменение денег' },
...
function getTTMValue(factsData, metricName, scale) {
  ...
  if (ttmType === 'last') {
    const values = getMetricValuesArray(factsData, metricName);
  ...
  // 4. Получаем значения через getQuarterValue для каждого квартала
  let sum = 0;
  let validCount = 0;
  
  for (const q of quarters) {
    const quarterParam = `q${q.quarterNum}`;
    const value = getQuarterValue(factsData, metricName, q.year, quarterParam);
